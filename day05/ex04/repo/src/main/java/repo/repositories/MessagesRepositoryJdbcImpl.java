...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) throws NotSavedSubEntityException {
        ...
        String query = "UPDATE messages SET text = ? , created_at = ? WHERE id = ?";
        try(Connection conn = dataSource.getConnection();
            PreparedStatement stmt = conn.prepareStatement(query)){
                ...
                if(rowUpdate == 0)
                    throw new NotSavedSubEntityException("No message found with the specified ID.");
        } catch(SQLException e){
            throw new NotSavedSubEntityException("SQL error occurred while updating the message.");
        }
    }
}
