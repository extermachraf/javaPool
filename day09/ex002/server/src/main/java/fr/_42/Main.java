...
public class Main {
    public static void main(String[] args) {
        ...
        Server server = new Server(Integer.parseInt(args[0].substring("--port=".length())), context.getBean(UsersServiceImpl.class), context.getBean(MessageRepositoryImpl.class), context.getBean(RoomRepositoryImpl.class));
        try {
            server.StartServer();
