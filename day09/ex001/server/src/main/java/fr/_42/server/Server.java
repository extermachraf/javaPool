...
public class Server {
    ...
    public Server(int port, UsersServiceImpl usersServiceImpl, MessageRepositoryImpl messageRepository) {
        this.port = port;
    ...
    private void handleClientConnection(ServerSocket serverSocket){
        try{
            while (!serverSocket.isClosed()) {
                ...
                Thread clientThread = new Thread(new ClientHandler(clientSocket, this.usersServiceImpl, this.messageRepository));
                clientThread.start();
            }
