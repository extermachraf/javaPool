...
public class ServerHandlers {
    ...
    public void disconnect() {
        System.out.println("problem happen in the server closing connection ...");
    ...
    private String processServerResponse(String response) {
        // Process different response types and format for display
        if (response.startsWith("ERROR")) {
            return "[Error] " + response.substring(6);
        }
        if (response.startsWith("AUTH_SUCCESS")) {
            return "\n=== " + response.substring(13) + " ===\n";
        }
        // Add more response processing as needed
        return response;
    }
}
